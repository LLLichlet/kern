...
Kern 不提供垃圾回收，不使用异常，也不会在你没有要求时分配内存。运行时策略需要写在项目配置和源码里；`std` 只是建立在 `base`、`sys`、`rt` 之上的库层，而不是编译器强制附带的一部分。
...
- [Runtime And Library Architecture](docs/runtime-architecture.md)：`base` / `sys` / `rt` / `std` 分层和 freestanding 模型。
