# Kern Brand Assets

This directory contains the canonical Kern visual assets.

- `kern-logo.svg`: full logo for README and documentation surfaces.
- `kern-mark-light.svg`: square mark for light-theme editor icons.
- `kern-mark-dark.svg`: square mark for dark-theme editor icons.
- `kern-mark.png`: PNG rendering of the light-theme mark for packaged extension metadata.

Generated or packaged copies may live under tool-specific directories, such as
`editors/vscode/icons/`, but those copies should be refreshed from this
directory rather than edited directly.

The editor mark assets are optically aligned for small file-icon slots. Their
viewBox is intentionally not a pure geometric crop of the paths.
