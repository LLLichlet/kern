...
fn main() i32 {
    ...
    let removed = match (items.remove(2)) {
        .{ Some: value } => value,
        .None => return 5,
    };
    if (removed != 2) {
    ...
    view.[1] = 8;
    items.truncate(4);
    if (items.as_slice() != [4]i32.{1, 8, 3, 1}) {
        return 16;
    }
    if (!items.first().is_some_and([](value: i32) bool { return value == 1; })) {
        return 17;
    }
    if (!items.last().is_some_and([](value: i32) bool { return value == 1; })) {
        return 18;
    }
    if (!items.contains([2]i32.{8, 3})) {
        return 19;
    }
    if (items.lex_cmp([4]i32.{1, 8, 3, 2}) != LESS) {
        return 20;
    }

    let first_big = match (items.position([](value: i32) bool { return value > 2; })) {
        .{ Some: index } => index,
        .None => return 21,
    };
    if (first_big != 1) {
    ...
    let last_big = match (items.rposition([](value: i32) bool { return value > 2; })) {
        .{ Some: index } => index,
        .None => return 23,
    };
    if (last_big != 2) {
    ...
    if (!items.any([](value: i32) bool { return value == 8; })) {
        return 25;
    }
    if (items.all([](value: i32) bool { return value < 8; })) {
        return 26;
    }
    let stripped_prefix = match (items.strip_prefix([2]i32.{1, 8})) {
        .{ Some: tail } => tail,
        .None => return 27,
    };
    ...
    let stripped_suffix = match (items.strip_suffix([2]i32.{3, 1})) {
        .{ Some: head } => head,
        .None => return 29,
    };
    ...
    items.reverse();
    if (items.as_slice() != [4]i32.{1, 3, 8, 1}) {
        return 31;
    }
    ...
    if (items.count([](value: i32) bool { return value >= 3; }) != 2) {
        return 36;
    }
    let mapped_big = match (items.find_map([](value: i32) ?i32 {
        if (value > 3) {
...
const CHECKED_SUM = usize.{40}.checked_add(2);
const CHECKED_OVERFLOW = num.USIZE_MAX.checked_add(1);
const CONST_ALIGNED = usize.{17}.align_up(8);
...
fn main() i32 {
    ...
    if (!usize.{10}.checked_sub(3).is_some_and([](value: usize) bool {
        return value == 7;
    })) {
        return 4;
    }
    if (usize.{10}.align_up(3).is_some()) {
        return 5;
    }
    if (!usize.{64}.is_power_of_two() or usize.{0}.is_power_of_two()) {
        return 6;
    }
    if (i32.{8}.min(3) != 3 or i32.{8}.max(3) != 8) {
        return 7;
    }
    if (i32.{19}.clamp(-2, 9) != 9 or i32.{-5}.clamp(-2, 9) != -2) {
        return 8;
    }
...
fn main() i32 {
    ...
    let mut items_seen = i32.{0};
    items.for_each([items_seen = items_seen..&](value: i32) void {
        items_seen.* += value;
    });
    if (items_seen != 10) {
        return 12;
    ...
    let doubled = items.fold(i32.{0}, [](accum: i32, value: i32) i32 {
    ...
    items.for_each_mut([](value: &mut i32) void {
        value.* *= 2;
    });
    if (items.as_slice() != [4]i32.{2, 4, 6, 8}) {
        return 14;
    }

    items.fill(7);
    if (items.as_slice() != [4]i32.{7, 7, 7, 7}) {
        return 15;
    }
    ...
    items.retain_mut([](value: &mut i32) bool {
        value.* *= 10;
        return value.* >= 30;
    });
    if (items.as_slice() != [5]i32.{40, 30, 30, 60, 60}) {
        return 29;
    }

    let swapped = match (items.swap_remove(1)) {
        .{ Some: value } => value,
        .None => return 30,
    };
    if (swapped != 30) {
    ...
    if (items.as_slice() != [4]i32.{40, 60, 30, 60}) {
        return 32;
    }

    items.rotate_left(1);
    if (items.as_slice() != [4]i32.{60, 30, 60, 40}) {
        return 100;
    }
    items.rotate_right(2);
    if (items.as_slice() != [4]i32.{60, 40, 60, 30}) {
        return 101;
    }
    if (!items.copy_within(0, 2, 2)) {
        return 102;
    }
    if (items.as_slice() != [4]i32.{60, 40, 60, 40}) {
        return 103;
    }
    ...
    if (items.as_slice() != [4]i32.{30, 40, 60, 60}) {
        return 34;
    }
    if (items.lower_bound(60) != 2 or items.upper_bound(60) != 4) {
        return 35;
    }
    if (!items.binary_search(40).is_some_and([](index: usize) bool {
        return index == 1;
    })) {
        return 36;
    }
    ...
    items.dedup();
    if (items.as_slice() != [4]i32.{1, 2, 3, 1}) {
        return 107;
    }
    items.dedup_by([](lhs: i32, rhs: i32) bool {
        return (lhs % 2) == (rhs % 2);
    });
    ...
    items.sort_by([](lhs: i32, rhs: i32) Ordering {
        if (lhs.cmp(rhs) == GREATER) {
            return Ordering.{-1};
        }
        return rhs.cmp(lhs);
    });
    if (items.as_slice() != [4]i32.{60, 60, 40, 30}) {
        return 37;
    }
...
fn main() i32 {
    ...
    if (count_range..&.count() != 5) {
        return 11;
    }

    let mut nth_range = range(10, 20);
    let third = match (nth_range..&.nth(usize.{3})) {
        .{ Some: value } => value,
        .None => return 12,
    };
    if (third != 13) {
    ...
    if (fold_range..&.fold[i32](0, [](accum: i32, value: i32) i32 {
        return accum + value;
    }) != 10) {
        return 16;
    }
    ...
    if (!any_range..&.any([](value: i32) bool { return value == 4; })) {
        return 17;
    }

    let mut all_range = range(1, 4);
    if (!all_range..&.all([](value: i32) bool { return value > 0; })) {
        return 18;
    }

    let mut find_range = range(3, 9);
    let found = match (find_range..&.find([](value: i32) bool { return value % 2 == 0; })) {
        .{ Some: value } => value,
        .None => return 19,
    };
    if (found != 4) {
    ...
    let pos = match (pos_range..&.position([](value: i32) bool { return value == 6; })) {
        .{ Some: value } => value,
        .None => return 21,
    };
    if (pos != usize.{3}) {
    ...
    let mapped = match (map_range..&.find_map[i32]([](value: i32) ?i32 {
    ...
    each_range..&.for_each([sum = each_sum..&](value: i32) void {
        sum.* += value;
    });
    if (each_sum != 6) {
        return 25;
    ...
    let last = match (last_range..&.last()) {
        .{ Some: value } => value,
        .None => return 26,
    };
    if (last != 1) {
