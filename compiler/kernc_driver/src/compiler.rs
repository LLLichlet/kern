...
impl CompilerDriver {
    ...
    fn sync_source_overrides(&self, source_overrides: &SourceOverrides) {
        self.frontend.sync_source_overrides(source_overrides);
    }

    fn structure_cache_key(
        &self,
        input_file: &str,
        source_overrides: &SourceOverrides,
    ) -> StructureCacheKey {
...
mod tests {
    ...
    fn count_assignments_in_expr(expr: &MastExpr) -> usize {
        ...
        let child_count = match &expr.kind {
            ...
            | MastExprKind::DataInit { payload: value, .. } => count_assignments_in_expr(value),
            ...
            MastExprKind::Return(value) => {
                value.as_deref().map(count_assignments_in_expr).unwrap_or(0)
            }
    ...
    fn analysis_artifact_exposes_flow_use_defs() {
        ...
        let use_def = function_owner
            ...
            .iter()
            .find(|use_def| {
                use_def.node_id == use_node_id && use_def.binding_id == value_binding.id
            })
    ...
    fn analysis_artifact_exposes_flow_single_source_uses() {
        ...
        let single_source = function_owner
            ...
            .iter()
            .find(|single| {
                single.node_id == final_value_use && single.binding_id == value_binding.id
            })
