...
//! Control-flow and data-flow facts used by diagnostics and lowering hints.
//!
//! This crate stores an analysis-oriented CFG over source-level nodes, then
//! derives reaching definitions, liveness, use-def chains, and lightweight
//! lowering hints.  The public structures are serializable-style facts; the
//! heavier computed bitsets stay private in `dataflow`.
...
pub struct AnalysisFlowCfg {
    /// Synthetic start node for forward data-flow passes.
    pub entry: AnalysisFlowNodeId,
    /// Synthetic terminal node reached by all normal exits.
...
pub struct AnalysisFlowDefinitionRef {
    /// Binding whose value is being defined.
    pub binding_id: AnalysisFlowBindingId,
    /// CFG node where that definition occurs.
    pub node_id: AnalysisFlowNodeId,
}
...
pub struct AnalysisFlowNodeTransfer {
    pub node_id: AnalysisFlowNodeId,
    /// Bindings read before this node's definitions take effect.
    pub use_binding_ids: Vec<AnalysisFlowBindingId>,
    /// Existing definitions killed by this node.
    pub kill_binding_ids: Vec<AnalysisFlowBindingId>,
    /// New definitions generated by this node.
...
impl FlowOwnerFacts {
    pub fn public_view(&self) -> AnalysisFlowOwner {
        // Keep cached implementation-only data such as `computed_liveness` out
        // of the stable public view returned to tools.
        AnalysisFlowOwner {
...
pub struct FlowLoweringElisionHints {
    /// Pure initializer expressions whose values are never observed.
    pub pure_dead_initializer_expr_ids: HashSet<NodeId>,
    /// Pure assignment RHS expressions whose stores are dead.
    pub pure_dead_assignment_expr_ids: HashSet<NodeId>,
    /// Binding expressions that can be omitted once their value is forwarded.
