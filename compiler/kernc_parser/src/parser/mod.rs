...
impl<'a> Parser<'a> {
    ...
    pub fn new_without_docs(source: &'a str, file_id: FileId, session: &'a mut Session) -> Self {
        Self::new_with_options(
            source,
            file_id,
            session,
    ...
    fn describe_token(&self, token: Token) -> String {
        match token.tag {
            ...
            _ => {
                let text = self.source_slice(token.span).to_string();
                if text.is_empty() {
                    format!("{:?}", token.tag)
                } else {
                    text
    ...
    fn doc_text_for_token(&self, token: Token, is_inner: bool) -> String {
        let source = self.source_slice(token.span);
