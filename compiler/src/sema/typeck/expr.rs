...
impl<'a> ExprChecker<'a> {
    ...
    pub fn check_expr(&mut self, expr: &Expr, expected_ty: Option<TypeId>) -> TypeId {
        let ty = match &expr.kind {
            ...
            // === 3. 声明与绑定 ===
            ExprKind::Let { name, init } => self.check_let_or_static(expr.id, *name, init, expected_ty, false, expr.span),
            ExprKind::Static { name, init } => self.check_let_or_static(expr.id, *name, init, expected_ty, true, expr.span),
            ...
            ExprKind::SliceOp { lhs, start, end, is_inclusive } => 
                self.check_slice_op(lhs, start.as_deref(), end.as_deref(), *is_inclusive, expr.span),
            ...
            ExprKind::EnumLiteral(variant_name) => self.check_enum_literal(*variant_name, expected_ty, expr.span),
            ExprKind::Undef => self.check_undef(expected_ty, expr.span),

            // === 9. 控制流 ===
            ExprKind::Block { stmts, result } => self.check_block(stmts, result.as_deref(), expected_ty),
            ExprKind::If { cond, then_branch, else_branch } => 
                self.check_if(cond, then_branch, else_branch.as_deref(), expected_ty),
            ExprKind::Switch { target, cases, default_case } => 
                self.check_switch_expr(target, cases, default_case.as_deref(), expected_ty, expr.span),
            ...
            // === 10. 泛型实例化 ===
            ExprKind::GenericInstantiation { target, types } => self.check_generic_instantiation(target, types, expr.span),
        };
    ...
    fn check_identifier(&mut self, name: crate::utils::SymbolId, span: Span) -> TypeId {
        if let Some(info) = self.ctx.scopes.resolve(name) {
            ...
            self.ctx.struct_error(span, format!("use of undeclared identifier `{}`", name_str))
    ...
    fn check_self_value(&mut self, span: Span) -> TypeId {
        ...
        if let Some(info) = self.ctx.scopes.resolve(self_var) {
            ...
            self.ctx.struct_error(span, "`self` is not available in this context")
    ...
    fn check_let_or_static(&mut self, node_id: ast::NodeId, name: crate::utils::SymbolId, init: &Expr, expected_ty: Option<TypeId>, is_static: bool, span: Span) -> TypeId {
    ...
    fn check_index_access(&mut self, lhs: &Expr, index: &Expr, span: Span) -> TypeId {
        ...
        if !self.ctx.type_registry.is_integer(norm_idx) && norm_idx != TypeId::ERROR {
            ...
            self.ctx.struct_error(index.span, "index must be an integer type")
        ...
        match self.ctx.type_registry.get(norm_lhs) {
            ...
            _ => {
                ...
                self.ctx.struct_error(lhs.span, "cannot index into a non-array/non-slice type")
    ...
    fn check_slice_op(&mut self, lhs: &Expr, start: Option<&Expr>, end: Option<&Expr>, _is_inclusive: bool, _span: Span) -> TypeId {
        let lhs_ty = self.check_expr(lhs, None);
        ...
        match self.ctx.type_registry.get(norm_lhs) {
            ...
            _ => {
                let lhs_str = self.ctx.ty_to_string(lhs_ty);
                self.ctx.struct_error(lhs.span, "cannot slice a non-array/non-slice type")
                    .with_hint(format!("target type is `{}`", lhs_str))
                    .emit();
                TypeId::ERROR
            }
    ...
    fn check_data_init_expr(&mut self, type_node: Option<&ast::TypeNode>, literal: &ast::DataLiteralKind, expected_ty: Option<TypeId>, span: Span) -> TypeId {
    ...
    fn check_enum_literal(&mut self, variant_name: crate::utils::SymbolId, expected_ty: Option<TypeId>, span: Span) -> TypeId {
        ...
        if let Some(exp_ty) = expected_ty {
            ...
            if let TypeKind::Def(def_id, _) = self.ctx.type_registry.get(norm_exp) {
                if let Def::Enum(e) = &self.ctx.defs[def_id.0 as usize] {
                    if e.variants.iter().any(|v| v.name == variant_name) {
                        ...
                        let available_variants: Vec<String> = e.variants.iter()
                    ...
                    let exp_str = self.ctx.ty_to_string(norm_exp);
                    self.ctx.struct_error(span, "expected an enum type for variant literal")
            ...
            self.ctx.struct_error(span, "cannot infer enum type for variant literal without context")
    ...
    fn check_undef(&mut self, expected_ty: Option<TypeId>, span: Span) -> TypeId {
        if expected_ty.is_none() {
            self.ctx.struct_error(span, "`undef` must have a known expected type context").emit();
            TypeId::ERROR
        } else {
            expected_ty.unwrap()
        }
    ...
    fn check_block(&mut self, stmts: &[ast::Stmt], result: Option<&Expr>, expected_ty: Option<TypeId>) -> TypeId {
    ...
    fn check_if(&mut self, cond: &Expr, then_branch: &Expr, else_branch: Option<&Expr>, expected_ty: Option<TypeId>) -> TypeId {
    ...
    fn check_switch_expr(&mut self, target: &Expr, cases: &[ast::SwitchCase], default_case: Option<&Expr>, expected_ty: Option<TypeId>, span: Span) -> TypeId {
    ...
    fn check_for(&mut self, init: Option<&Expr>, cond: Option<&Expr>, post: Option<&Expr>, body: &Expr) -> TypeId {
        self.ctx.scopes.enter_scope();
    ...
    fn check_return(&mut self, val: Option<&Expr>, span: Span) -> TypeId {
        ...
        if let Some(v) = val {
            ...
            if expected_ret != TypeId::VOID && expected_ret != TypeId::ERROR {
                ...
                self.ctx.struct_error(span, "expected a return value, but found empty return")
    ...
    fn check_generic_instantiation(&mut self, target: &Expr, types: &[ast::TypeNode], span: Span) -> TypeId {
        let target_ty = self.check_expr(target, None);
        ...
        let (def_id, _old_args) = match self.ctx.type_registry.get(target_norm) {
            ...
            _ => {
                self.ctx.struct_error(span, "this expression does not support generic instantiation").emit();
                return TypeId::ERROR;
            }
        };
        ...
        if generics.len() != arg_tys.len() {
            self.ctx.struct_error(span, format!("expected {} generic arguments, but {} were provided", generics.len(), arg_tys.len())).emit();
            return TypeId::ERROR;
        }
            ...
            for constraint_node in &param.constraints {
                ...
                if constraint_ty != TypeId::ERROR {
                    if !self.check_trait_impl(act_ty, constraint_ty) {
                        ...
                        self.ctx.struct_error(span, format!("type does not satisfy trait bounds for generic parameter `{}`", param_name))
        ...
        match op {
            ...
            _ => { // Bitwise Ops
                if !self.ctx.type_registry.is_integer(l_norm) {
                    self.ctx.struct_error(lhs.span, "bitwise operations require integer types").emit();
                }
                if !self.check_coercion(rhs.span, l_norm, r_norm) { return TypeId::ERROR; }
                l_norm
            }
        ...
        match op {
            ...
            UnaryOperator::PointerDeRef => {
                ...
                match self.ctx.type_registry.get(norm) {
                    ...
                    _ => {
                        ...
                        self.ctx.struct_error(span, "cannot dereference a non-pointer type")
            ...
            UnaryOperator::LengthOf => {
                ...
                match self.ctx.type_registry.get(norm) {
                    ...
                    _ => {
                        self.ctx.struct_error(span, "length operator `#` can only be applied to arrays and slices").emit();
                        TypeId::ERROR
                    }
                }
            }
            UnaryOperator::Negate => {
                if !self.ctx.type_registry.is_integer(self.strip_mut(op_ty)) && !self.ctx.type_registry.is_float(self.strip_mut(op_ty)) {
                    self.ctx.struct_error(span, "negation requires a numeric type").emit();
                }
                op_ty
            }
            ...
            UnaryOperator::BitwiseNot => {
                if !self.ctx.type_registry.is_integer(self.strip_mut(op_ty)) {
                    self.ctx.struct_error(span, "bitwise NOT requires an integer type").emit();
                }
                op_ty
            }
            _ => TypeId::ERROR,
    ...
    fn check_array_decay(&mut self, exp_elem: TypeId, act_kind: &TypeKind, span: Span) -> Result<bool, ()> {
    ...
    fn emit_mismatch_error(&mut self, span: Span, expected: TypeId, actual: TypeId) {
        ...
        self.ctx.struct_error(span, "mismatched types")
    ...
    fn check_cast(&mut self, span: Span, from: TypeId, to: TypeId) {
        ...
        if let TypeKind::TraitObject(to_def_id, _) = self.ctx.type_registry.get(t_norm) {    
            ...
            if is_to_mut && !is_from_mut_ptr {
                self.ctx.struct_error(span, "cannot cast a read-only pointer to a mutable trait object `mut Trait`")
            ...
            let is_from_trait_obj = matches!(self.ctx.type_registry.get(f_norm), TypeKind::TraitObject(..));

            if !is_from_ptr && !is_from_trait_obj {
                let from_str = self.ctx.ty_to_string(from);
                self.ctx.struct_error(span, "only pointers or trait objects can be cast to a trait object")
            ...
            if !self.check_trait_impl(from, t_norm) { 
                ...
                self.ctx.struct_error(span, format!("the source type does not implement trait `{}`", trait_name)).emit();
            }
            return;
        }
    ...
    fn check_field_access(&mut self, lhs: &Expr, field: crate::utils::SymbolId, span: Span) -> TypeId {
        ...
        if let TypeKind::Def(def_id, generic_args) = self.ctx.type_registry.get(current_norm).clone() {
            if let Some(field_ty) = self.resolve_def_field(def_id, &generic_args, field, is_target_mut) {
                return field_ty;
            }
        }
        ...
        self.ctx.struct_error(span, format!("no field or method named `{}` found on type `{}`", field_str, lhs_str))
    ...
    fn resolve_trait_object_method(&mut self, trait_def_id: crate::sema::ty::DefId, trait_args: &[TypeId], field: crate::utils::SymbolId, span: Span) -> TypeId {
        let trait_def = match &self.ctx.defs[trait_def_id.0 as usize] {
            Def::Trait(t) => t.clone(),
            _ => unreachable!(),
        };

        if let Some(&(_, mut method_ty)) = trait_def.resolved_methods.iter().find(|(m_name, _)| *m_name == field) {
            // 泛型实例化替换
        ...
        let field_str = self.ctx.resolve(field);
        self.ctx.struct_error(span, format!("method `{}` not found in trait object", field_str))
    ...
    fn resolve_def_field(&mut self, def_id: crate::sema::ty::DefId, generic_args: &[TypeId], field: crate::utils::SymbolId, is_target_mut: bool) -> Option<TypeId> {
        let def = self.ctx.defs[def_id.0 as usize].clone();

        match &def {
            Def::Struct(s) => {
                if let Some(f) = s.fields.iter().find(|f| f.name == field) {
                    return Some(self.apply_generics_and_mut(&s.generics, generic_args, f.type_node.id, is_target_mut));
                }
            }
            Def::Union(u) => {
                if let Some(f) = u.fields.iter().find(|f| f.name == field) {
                    return Some(self.apply_generics_and_mut(&u.generics, generic_args, f.type_node.id, is_target_mut));
                }
            }
            Def::Enum(e) => {
                if e.variants.iter().any(|v| v.name == field) {
                    // 访问 Enum 变体（如 `Status.Ok`），返回 Enum 本身的类型
                    return Some(self.ctx.type_registry.intern(TypeKind::Def(def_id, generic_args.to_vec())));
                }
            }
            _ => {}
        }
    ...
    fn apply_generics_and_mut(&mut self, generics: &[ast::GenericParam], args: &[TypeId], node_id: ast::NodeId, is_target_mut: bool) -> TypeId {
        let mut field_ty = self.ctx.node_types.get(&node_id).copied().unwrap_or(TypeId::ERROR);
    ...
    fn resolve_impl_method(&mut self, lhs_ty: TypeId, field: crate::utils::SymbolId) -> Option<TypeId> {
        ...
        found_method_id.map(|method_id| {
            self.ctx.type_registry.intern(TypeKind::FnDef(method_id, resolved_impl_args))
        })
    }
    ...
    fn resolve_method_context(&self, callee: &Expr) -> (bool, TypeId) {
        if let ExprKind::FieldAccess { lhs, .. } = &callee.kind {
            ...
            if matches!(self.ctx.type_registry.get(norm_node_ty), TypeKind::FnDef(..) | TypeKind::Function {..}) {
                let receiver_ty = self.ctx.node_types.get(&lhs.id).copied().unwrap_or(TypeId::ERROR);
    ...
    /// 助手 3：校验参数个数 (Arity)
    fn check_call_arity(&mut self, arg_count: usize, param_count: usize, is_method: bool, is_variadic: bool, span: Span) {
        let expected_arg_count = if is_method { param_count.saturating_sub(1) } else { param_count };
        
        if is_variadic {
            if arg_count < expected_arg_count {
                self.ctx.struct_error(span, format!("function expects at least {} arguments, but {} were provided", expected_arg_count, arg_count)).emit();
            }
        } else {
            if arg_count != expected_arg_count {
                self.ctx.struct_error(span, format!("function expects exactly {} arguments, but {} were provided", expected_arg_count, arg_count)).emit();
            }
        }
    }
    ...
    fn check_method_receiver(&mut self, expected_self: TypeId, receiver_ty: TypeId, span: Span) {
        if !self.check_coercion(span, expected_self, receiver_ty) {
            ...
            let is_exp_ptr = matches!(self.ctx.type_registry.get(norm_expected), TypeKind::Pointer(_) | TypeKind::VolatilePtr(_));
    ...
    fn check_call_arguments(&mut self, args: &[Expr], params: &[TypeId], is_method: bool, _is_variadic: bool) {
        ...
        for (i, arg) in args.iter().enumerate() {
            ...
            if sig_param_idx < params.len() {
                ...
                let is_small_int = matches!(norm_arg, TypeId::I8 | TypeId::I16 | TypeId::U8 | TypeId::U16);
                ...
                if is_small_int {
                    ...
                    self.ctx.struct_error(arg.span, "C ABI requires float arguments passed to `...` to be 64-bit")
    ...
    fn check_array_literal(&mut self, elems: &[Expr], expected: TypeId, exp_norm: TypeId, span: Span) -> TypeId {
        if let TypeKind::Array { elem: exp_elem, len } = self.ctx.type_registry.get(exp_norm) {
            ...
            if elems.len() as u64 != *len {
                self.ctx.struct_error(span, format!("array literal length ({}) does not match expected length ({})", elems.len(), len)).emit();
            }
            ...
            self.ctx.struct_error(span, "expected an array type for array literal `.{ ... }`")
    ...
    fn check_repeat_literal(&mut self, value: &Expr, count: &Expr, expected: TypeId, exp_norm: TypeId, span: Span) -> TypeId {
        if let TypeKind::Array { elem: exp_elem, .. } = self.ctx.type_registry.get(exp_norm) {
            ...
            if !self.ctx.type_registry.is_integer(self.strip_mut(c_ty)) {
                self.ctx.struct_error(count.span, "repeat count must be an integer").emit();
            }
            expected
        } else {
            let ty_str = self.ctx.ty_to_string(expected);
            self.ctx.struct_error(span, "expected an array type for repeat literal `.{ v; N }`")
                .with_hint(format!("context expects `{}`", ty_str))
                .emit();
            TypeId::ERROR
        }
                        ...
                        if visited.insert(*inst_def_id) {
                            if let Def::Trait(trait_def) = self.ctx.defs[inst_def_id.0 as usize].clone() {
                                for supertrait_ast in &trait_def.supertraits {
                                    ...
                                    if inst_super_ty == target_trait_ty || self.check_trait_impl_inner(source_ty, inst_super_ty, visited) {
                                        return true;
                                    }
                                }
                            }
    ...
    fn check_switch_exhaustiveness(&mut self, target_ty: TypeId, cases: &[ast::SwitchCase], has_default: bool, span: Span) {
        ...
        if let TypeKind::Def(def_id, _) = self.ctx.type_registry.get(norm_target) {
            if let Def::Enum(e) = &self.ctx.defs[def_id.0 as usize] {
                ...
                if !unhandled_variants.is_empty() {
                    ...
                    self.ctx.struct_error(span, "switch expression is not exhaustive")
        ...
        self.ctx.struct_error(span, "switch expression must be exhaustive")
