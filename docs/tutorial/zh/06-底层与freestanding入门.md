...
- hosted：程序运行在操作系统进程环境里，通常使用 `std`、`sys` 和工具链 startup。
...
- 不隐式链接 libc。
- 接入官方 `std` bundle，也就是 `base`、`sys`、`std` 等 root alias。

这也是 Kern 和很多 C/C++ 项目的一个关键差异：hosted 不等于“依赖 libc”。Kern 的 hosted 能力通过 `sys` 接入 OS/provider 边界，`std` 建立在 `base` 和 `sys` 上；libc 只是需要兼容 C ABI、链接外部 C 库或使用平台 C runtime 时才显式选择。
...
- `std`：接入普通 hosted 项目常用的 `base`、`sys`、`std`。
