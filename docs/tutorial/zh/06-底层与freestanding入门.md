...
- hosted：程序运行在操作系统进程环境里，通常使用 `std`、`prov` 和工具链 startup。
...
- 不隐式链接 libc。
- 接入官方 `std` bundle，也就是 `base`、`prov`、`std` 等 root alias。

这也是 Kern 和很多 C/C++ 项目的一个关键差异：hosted 不等于“依赖 libc”。Kern 的 hosted 能力通过 `prov` 合约和 `std.host` 接入 hosted 能力，`std` 建立在 `base` 和 `prov` 上；libc 只是需要兼容 C ABI、链接外部 C 库或使用平台 C runtime 时才显式选择。
...
- `std`：接入普通 hosted 项目常用的 `base`、`prov`、`std`。
...
`entry = "none"` 不代表“不能用库”。它只表示工具链不接管 startup。你仍然可以使用 `base` 里的整数、切片、比较、布局、allocator trait、集合等 freestanding 设施；只是任何需要 OS/provider 的能力都必须由项目提供边界。
...
这一章的核心不是让你马上写完整 kernel，而是建立边界感：启动、链接、内存、OS/provider 和 libc 都是项目策略；Kern 不把这些策略藏进语言默认行为里。
