...
标准库的 OS provider 大量使用这种模式。条件编译的核心效果是“裁剪源码节点”，不是运行时 `if`，因此被裁掉的分支不参与后续语义检查。
...
继续深入时，可以阅读 [`design.md`](../../design.md) 的 attributes、inline assembly 和 compiler intrinsics 章节。标准库里的 [`library/sys/os/linux.rn`](../../../library/sys/os/linux.rn)、[`library/base/sync/init.rn`](../../../library/base/sync/init.rn) 也很适合配合阅读。
