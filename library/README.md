...
- `base`: freestanding primitives, traits, memory helpers, collections, and
  numeric utilities.
- `sys`: provider and operating-system boundary APIs built on `base`.
- `std`: user-facing facilities built on `base` and `sys`.
...
library paths (`KERN_BASE_PATH`, `KERN_SYS_PATH`, `KERN_STD_PATH`, and
