...
mod tests {
    ...
    fn archive_kind_accepts_release_archive_extensions() {
        assert_eq!(
            archive_kind_from_path(Path::new("kern-v0.7.9-x86_64-linux-gnu.tar.gz")).unwrap(),
            ArchiveKind::TarGz
        );
        assert_eq!(
            archive_kind_from_path(Path::new("kern-v0.7.9-x86_64-windows-msvc.zip")).unwrap(),
            ArchiveKind::Zip
        );
    ...
    fn load_workspace_version_reads_workspace_package_section() {
        ...
        assert_eq!(load_workspace_version(&root).unwrap(), "0.7.9");
