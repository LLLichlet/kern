...
use shared_ops::{
    ...
    OpsResult, archive_kind_from_path, artifact_record_json, copy_dir_recursive, copy_path,
    detect_host_target, expected_archive_sha256, extract_archive_with_system_tool, file_size,
    format_policy_value, load_workspace_version, make_temp_dir, remove_path_if_exists, repo_root,
    resolve_bundled_toolchain, resolve_ci_toolchain_policy, resolve_official_library_root,
    run_command, run_command_capture, run_command_with_env, runner_os_for_host,
    runner_os_for_target, sdk_manifest_json, sha256_directory, sha256_file,
    toolchain_manifest_json, validate_toolchain_root, verify_archive_checksum, write_json_value,
};
use std::env;
...
struct ReleasePackageArgs {
    ...
    target: Option<String>,
    skip_build: bool,
    toolchain_prefix: Option<PathBuf>,
}

#[derive(Debug, Default)]
struct ReleaseToolchainPackageArgs {
    version: Option<String>,
    target: Option<String>,
    toolchain_prefix: Option<PathBuf>,
}
...
fn run() -> OpsResult<()> {
    match parse_args(env::args().skip(1).collect())? {
        ...
        Command::Ci(CiCommand::VerifyVsix(args)) => verify_vscode_extension_archive(args),
        Command::Release(ReleaseCommand::Package(args)) => package_release(args),
        Command::Release(ReleaseCommand::PackageToolchain(args)) => package_toolchain_release(args),
        Command::Release(ReleaseCommand::WriteChecksums(args)) => write_release_checksums(args),
        Command::Release(ReleaseCommand::Help) => {
            print!("{}", release_help().render(ColorChoice::Auto));
            Ok(())
        }
...
fn verify_vscode_extension_archive(args: VsixVerifyArgs) -> OpsResult<()> {
...
fn package_release(args: ReleasePackageArgs) -> OpsResult<()> {
    let root = repo_root()?;
    let host = detect_host_target()?;
    let version = args
        .version
...
fn package_toolchain_release(args: ReleaseToolchainPackageArgs) -> OpsResult<()> {
    let root = repo_root()?;
    let host = detect_host_target()?;
    let archive_target = args.target.unwrap_or_else(|| host.archive_target.clone());
    ensure_host_native_target(&archive_target, &host)?;
...
fn write_release_checksums(args: ReleaseChecksumsArgs) -> OpsResult<()> {
    let root = repo_root()?;
