...
fn push_bounded_diagnostic(bundle: &mut Vec<IdeDiagnostic>, diagnostic: IdeDiagnostic) {
...
fn diagnostic_severity(level: kernc_utils::DiagnosticLevel) -> IdeDiagnosticSeverity {
    match level {
        kernc_utils::DiagnosticLevel::Error | kernc_utils::DiagnosticLevel::Ice => {
            IdeDiagnosticSeverity::Error
        }
        kernc_utils::DiagnosticLevel::Warning => IdeDiagnosticSeverity::Warning,
        kernc_utils::DiagnosticLevel::Note => IdeDiagnosticSeverity::Information,
    }
}
...
fn diagnostic_tags(diagnostic: &kernc_utils::Diagnostic) -> Vec<IdeDiagnosticTag> {
    let mut tags = diagnostic
        .tags
        .iter()
        .map(|tag| match tag {
            kernc_utils::DiagnosticTag::Unnecessary => IdeDiagnosticTag::Unnecessary,
            kernc_utils::DiagnosticTag::Deprecated => IdeDiagnosticTag::Deprecated,
        })
        .collect::<Vec<_>>();
    tags.sort();
    tags.dedup();
...
fn diagnostic_related_information(
    ...
    diagnostic: &kernc_utils::Diagnostic,
    uri_by_path: Option<&BTreeMap<PathBuf, String>>,
) -> Vec<IdeDiagnosticRelatedInformation> {
...
mod tests {
    ...
    use crate::analysis::ide::IdeDiagnosticTag;
    ...
    fn convert_diagnostic_includes_hints_and_related_information() {
        ...
        assert_eq!(converted.tags, vec![IdeDiagnosticTag::Unnecessary]);
        let related = converted.related_information.clone();
        assert_eq!(related.len(), 1);
        assert_eq!(related[0].message, "related here");
        assert_eq!(related[0].location.range.start.line, 0);
        assert_eq!(
            converted.into_lsp().tags,
